...
import { MAT_KEYBOARD_DEADKEYS, IKeyboardDeadkeys } from '../keyboard-deadkey-config';
import { MAT_KEYBOARD_ICONS, IKeyboardIcons } from '../keyboard-icons-config';
...
export class MatKeyboardKeyComponent implements OnInit {
  ...
  onClick(event: MouseEvent): void {
    ...
    switch (this.key) {
      ...
      case SpecialKey.Alt:
      case SpecialKey.AltGr:
      case SpecialKey.AltLk:
        this.altClick.emit(event);
        break;

      case SpecialKey.Bksp:
  ...
  private _getSelectionLength(): number {
    ...
    if ('selection' in window.document) {
      // IE
      this.input.nativeElement.focus();
      return this.document['selection'].createRange().text.length;
    }
  }
